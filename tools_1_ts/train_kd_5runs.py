"""
one stage
student classification trained by teacher, using kd loss
"""
import os
import sys
sys.path.insert(0, './')
...
import pprint
import argparse
from lib.core.utils import str_gpus, AverageMeter, accuracy
...
def creat_model_kd(cfg):
    ...
    from lib.models.model_builder import build_model, NaiveKDModelBuilder
    ...
    if cfg.MODEL.KDTYPE == 'ickd':
        crop_size_large = cfg.DATA.CROP_SIZE
        crop_size_small = int(cfg.DATA.CROP_SIZE / cfg.DATA.LESSEN_RATIO)
        input_l = torch.ones(1, 3, crop_size_large, crop_size_large)
        input_s = torch.ones(1, 3, crop_size_small, crop_size_small)
        cfg.ICKD.FEATDIM_T = list(model_t.get_feat_size(input_l))
        cfg.ICKD.FEATDIM_S = list(model_s.get_feat_size(input_s))

    if cfg.MODEL.KDTYPE == 'kd':
        model = NaiveKDModelBuilder(model_s, model_t)
    else:
        model = NaiveKDModelBuilder(model_s, model_t)
    ...
    kd_criterion_this = build_criterion(cfg).to(device)
    print('Preparing networks done!')
    return device, model, optimizer, scheduler, cls_criterion, pkd_criterion, kd_criterion_this
...
def main():
    ...
    cfg.BASIC.SAVE_DIR = os.path.join(
        ...
        '{}_{}_{}_{}_k{}_seed{}_{}_{}'.format(cfg.MODEL.KDTYPE, cfg.MODEL.ARCH_T, cfg.MODEL.ARCH_S,
                                               cfg.DATA.CROP_SIZE, cfg.DATA.LESSEN_RATIO, cfg.BASIC.SEED,
                                               cfg.SOLVER.START_LR, cfg.BASIC.TIME))
    ...
    for irun in range(1, 6):
        ...
        device, model, optimizer, scheduler, cls_criterion, pkd_criterion, kd_criterion_this = creat_model_kd(cfg)
        ...
        for epoch in range(1, cfg.SOLVER.NUM_EPOCHS+1):
            update_train_step = train_one_epoch(train_loader, model, device, cls_criterion, pkd_criterion,
                                                kd_criterion_this, optimizer, epoch, irun, logger_irun, cfg,
                                                update_train_step)
            scheduler.step()
...
def train_one_epoch(train_loader, model, device, cls_criterion, pkd_criterion, kd_criterion_this, optimizer,
    ...
    for i, (input_large, input_small, target, name) in enumerate(train_loader):
        ...
        if cfg.MODEL.KDTYPE == 'kd' or cfg.MODEL.KDTYPE == 'dkd':
            if cfg.MODEL.KDTYPE == 'kd':
                cls_logits_s, cls_logits_t = model(input_large, input_small)
                loss = pkd_criterion(cls_logits_s, cls_logits_t, target)
            elif cfg.MODEL.KDTYPE == 'dkd':
                cls_logits_s, cls_logits_t = model(input_large, input_small)
                cls_loss = cls_criterion(cls_logits_s, target)
                dkd_loss = kd_criterion_this(cls_logits_s, cls_logits_t, target, epoch+1)
                loss = cls_loss + dkd_loss
        else:
            cls_logits_s, feats_s, cls_logits_t, feats_t = model(input_large, input_small, return_feat=True)
            cls_loss = cls_criterion(cls_logits_s, target)
            if cfg.MODEL.KDTYPE == 'sp':
                fkd_loss = sum(kd_criterion_this([feats_s[-1]], [feats_t[-1]]))
                loss = cls_loss + fkd_loss * cfg.SP.BETA
            elif cfg.MODEL.KDTYPE == 'at':
                fkd_loss = sum(kd_criterion_this(feats_s[1:], feats_t[1:]))
                loss = cls_loss + fkd_loss * cfg.AT.BETA
            elif cfg.MODEL.KDTYPE == 'ickd':
                fkd_loss = sum(kd_criterion_this([feats_s[-1]], [feats_t[-1]]))
                loss = cls_loss + fkd_loss * cfg.ICKD.BETA
            else:
                raise ValueError('Please set correct knowledge distillation method.')
