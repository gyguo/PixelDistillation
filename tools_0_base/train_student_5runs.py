...
def main():
    ...
    cfg.BASIC.SAVE_DIR = os.path.join(cfg.BASIC.ROOT_DIR, 'ckpt', cfg.DATA.DATASET, '{}_{}_{}_{}_{}'.format(
        cfg.MODEL.TYPE, cfg.MODEL.ARCH, cfg.DATA.CROP_SIZE, cfg.SOLVER.START_LR, cfg.BASIC.TIME))
    ...
    for irun in range(1, 6):
        ...
        for epoch in range(1, cfg.SOLVER.NUM_EPOCHS+1):
            update_train_step = train_one_epoch(train_loader, model, device, criterion, optimizer,
                                                epoch, irun, logger_irun, cfg, update_train_step)
            scheduler.step()
            acc, update_val_step = val_one_epoch(val_loader, model, device, criterion, epoch, irun,
...
def train_one_epoch(train_loader, model, device, criterion, optimizer, epoch, irun, logger, cfg, update_train_step):
    losses = AverageMeter()
    eval = AverageMeter()
