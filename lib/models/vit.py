...
default_cfgs = {
    ...
    # size 448, use pretrained model of size 384
    'vit_base_patch16_448': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
            'B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_384.npz',
        input_size=(3, 448, 448), crop_pct=1.0),
    'deit_base_patch16_448': _cfg(
        url='https://dl.fbaipublicfiles.com/deit/deit_base_patch16_384-8de9b5d1.pth',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, input_size=(3, 448, 448), crop_pct=1.0),
    'deit_base_distilled_patch16_448': _cfg(
        url='https://dl.fbaipublicfiles.com/deit/deit_base_distilled_patch16_384-d0272ac0.pth',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, input_size=(3, 448, 448), crop_pct=1.0,
        classifier=('head', 'head_dist')),
}
...
def vit_base_patch16_448(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_448', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def deit_base_patch16_448(pretrained=False, **kwargs):
    """ DeiT base model @ 384x384 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('deit_base_patch16_448', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def deit_base_distilled_patch16_448(pretrained=False, **kwargs):
    """ DeiT-base distilled model @ 384x384 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        'deit_base_distilled_patch16_448', pretrained=pretrained, distilled=True, **model_kwargs)
    return model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_file', type=str, default='../../configs/cub/cub_vit.yaml',
    ...
    image = Image.open('../../demo/demo.JPEG').convert('RGB')
